...
cdef class NumpyArrayNodeRecorder_transmission_line_test(NodeRecorder):
    ...
    NumpyArrayNodeCurtailmentRatioRecorder
    """
    def __init__(self, model, AbstractNode node, factor, **kwargs):
        # Optional different method for aggregating across time.
        temporal_agg_func = kwargs.pop('temporal_agg_func', 'mean')
        self.factor = factor
        super(NumpyArrayNodeRecorder_transmission_line_test, self).__init__(model, node, **kwargs)
    ...
    cpdef setup(self):
        cdef int ncomb = len(self.model.scenarios.combinations)
        cdef int nts = len(self.model.timestepper)
        self.data = np.zeros((nts, ncomb), dtype=np.float64)
        self.data_ = np.zeros((ncomb), dtype=np.float64)
    ...
    cpdef after(self):
        ...
        for c in range(self.data.shape[1]):
            self.data[ts.index,c] = np.absolute(self._node.flow[c])

        if ts.year==self.model.timestepper.end.year and ts.month==12:
            t=self.data.max(axis=0)
            for i in range(self.data.shape[1]):
                temp=t[i]-self.factor[1]
                if temp <=0:
                    self.data_[i]=0.0
                else:
                    self.data_[i]=temp*self.factor[0]
            return self.data_
    ...
    def load(cls, model, data):
        ...
        rec = cls(model, node, factor, **data)
        return rec

NumpyArrayNodeRecorder_transmission_line_test.register()
...
cdef class AggregatedRecorder_Objective_cal_export(Recorder):
    """
    This Recorder is used to aggregate across multiple other Recorder objects.
    ...
    def __init__(self, model, recorders, **kwargs):
        ...
        super(AggregatedRecorder_Objective_cal_export, self).__init__(model, **kwargs)
    ...
    cpdef setup(self):
        ...
        cdef int ncomb = len(self.model.scenarios.combinations)
        cdef int nts = len(self.model.timestepper)
        self.data = np.zeros(ncomb, dtype=np.float64)

    cpdef after(self):
        cdef ScenarioIndex scenario_index
        for scenario_index in self.model.scenarios.combinations:
            self.data[scenario_index.global_id] = (self.data[scenario_index.global_id] + np.sum([recorder.flow[scenario_index.global_id]*1e-6 for recorder in self.recorders]))
    ...
    def load(cls, model, data):
        recorder_names = data.pop("recorders")
        recorders = [model._get_node_from_ref(model, node_name) for node_name in recorder_names] 
        rec = cls(model, recorders, **data)
        return rec

AggregatedRecorder_Objective_cal_export.register()
...
cdef class AggregatedRecorder_Objective_cal(Recorder):
    """
    This Recorder is used to aggregate across multiple other Recorder objects.
    ...
    def __init__(self, model, recorders, **kwargs):
        ...
        super(AggregatedRecorder_Objective_cal, self).__init__(model, **kwargs)
        self.recorders = recorders
    ...
    cpdef after(self):
        cdef ScenarioIndex scenario_index
        for scenario_index in self.model.scenarios.combinations:
            val=[recorder.flow[scenario_index.global_id] for recorder in self.recorders]
            self.data[scenario_index.global_id] = self.data[scenario_index.global_id] + np.sum(val)*1e-6
        return self.data
...
AggregatedRecorder_Objective_cal.register()
